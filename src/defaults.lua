package.preload['defaults'] = function()
    ...
    m.commands = {
        ...
        echo= function(arg) treest:message(dostring('return '..arg)) end,
        eval= function(arg) dostring(arg) end,
        help= function(arg) treest:message(help(arg) or "no help for "..arg) end,
        quit= function() treest:quit() end,
        suspend= function(arg) treest:suspend() end,
    }
    ...
    m.commands.sus = m.commands.suspend m.commands.st = m.commands.stop = m.commands.suspend

    local function search(flags)
        local ans = treest:prompt('/', function() end)
        if not ans then return end
        local found = treest:search(q, flags)
        if not found
          then treest:message("not found: "..api['/'])
          else
            treest:message(nil)
            treest:jumpto(found)
        end
    ...
    m.keys = {
        ...
        ['n']= function() search(treest:register('/'), {'next', 'sat'}) end,
        ['N']= function() search(treest:register('/'), {'prev', 'sat'}) end,
